...
export default function PlaceBid({ socket, isAuthenticated }) {
  ...
  const getCategoryName = async (category_id) => {
    try {
      const res = await Axios.get(
        `http://${process.env.REACT_APP_AUCTION_SERVER}/backend/auction/getcategory/${category_id}`
      );
  ...
  useEffect(() => {
    ...
    const fetchProduct = async (product_id) => {
      try {
        const res = await Axios.get(
          `http://${process.env.REACT_APP_AUCTION_SERVER}/backend/auction/displayproduct/${product_id}`
        );
        if (res.data) {
          setProduct(res.data);
    ...
    const getLastBid = async (product_id) => {
      try {
        const res = await Axios.get(
          `http://${process.env.REACT_APP_BID_SERVER}/backend/bid/${product_id}`
        );
  ...
  async function addBid(e) {
    ...
    await Axios.post(`http://${process.env.REACT_APP_BID_SERVER}/backend/bid/create`, {
